...
// Mock data theo wireframe
const MOCK_RECEIPTS = [
  {
    id: 1,
    receiptCode: 'REC-2023-001',
  ...
  {
    id: 2,
    receiptCode: 'REC-2023-002',
    supplier: 'Pioneer Parts Co',
    date: '2023-10-25',
    status: 'Pending',
  },
  {
    id: 3,
    receiptCode: 'REC-2023-003',
    supplier: 'Swift Delivery Ltd',
    date: '2023-10-25',
    status: 'In Transit',
  },
  {
    id: 4,
    receiptCode: 'REC-2023-004',
  ...
  {
    id: 5,
    receiptCode: 'REC-2023-005',
    supplier: 'Oceanic Supplies',
    date: '2023-10-27',
    status: 'Cancelled',
  },
  {
    id: 6,
    receiptCode: 'REC-2023-006',
    supplier: 'Northern Tools',
    date: '2023-10-27',
    status: 'Received',
  },
];
...
function InboundReceiptListContent() {
  const router = useRouter();

  const handleViewDetail = (receiptCode: string) => {
    router.push(`/inbound/${receiptCode}`);
  };
    ...
    <div className="max-w-7xl mx-auto w-full flex flex-col gap-6 p-4 md:p-8 bg-gray-50/50 min-h-screen font-sans">
      ...
      <div className="flex flex-col md:flex-row md:items-end justify-between gap-4">
        <div>
          ...
          <p className="mt-1 text-sm text-gray-500">
            Manage incoming warehouse receipts and deliveries.
          </p>
        </div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-4 gap-4">
        <div className="bg-white p-5 rounded-lg border border-gray-200 shadow-sm flex flex-col justify-center">
          <span className="text-xs font-semibold text-gray-500 uppercase tracking-wider">Total Receipts</span>
          <span className="text-3xl font-bold text-gray-900 mt-1">156</span>
        </div>
        <div className="bg-white p-5 rounded-lg border border-gray-200 shadow-sm flex flex-col justify-center">
          <span className="text-xs font-semibold text-gray-500 uppercase tracking-wider">Received</span>
          <span className="text-3xl font-bold text-green-600 mt-1">98</span>
        </div>
        <div className="bg-white p-5 rounded-lg border border-gray-200 shadow-sm flex flex-col justify-center">
          <span className="text-xs font-semibold text-gray-500 uppercase tracking-wider">Pending</span>
          <span className="text-3xl font-bold text-amber-500 mt-1">32</span>
        </div>
        <div className="bg-white p-5 rounded-lg border border-gray-200 shadow-sm flex flex-col justify-center">
          <span className="text-xs font-semibold text-gray-500 uppercase tracking-wider">In Transit</span>
          <span className="text-3xl font-bold text-blue-600 mt-1">26</span>
        </div>
      </div>
      ...
      <div className="bg-white rounded-lg border border-gray-200 shadow-sm overflow-hidden">
          ...
          <table className="w-full text-left border-collapse">
            ...
            <tbody className="divide-y divide-gray-200">
              {MOCK_RECEIPTS.map((receipt) => (
                ...
                >
                  <td
                    onClick={() => handleViewDetail(receipt.receiptCode)}
        ...
        <div className="px-6 py-4 border-t border-gray-200 flex items-center justify-between bg-white">
          <span className="text-sm text-gray-500">Showing 1 to 6 of 156 entries</span>
    ...
    <Suspense fallback={
      <div className="min-h-screen flex items-center justify-center bg-gray-50/50">
        <div className="flex flex-col items-center gap-3">
          ...
          <div className="text-sm font-medium text-gray-500">Loading Receipts...</div>
        </div>
      </div>
    }>
      <InboundReceiptListContent />
    </Suspense>
  );
}
