  ...
  return (
    <div style={{ marginTop:8 }}>
      <div style={{ display:'flex', gap:4, marginBottom:6 }}>
        {[0,1,2,3].map(i => (
          <div key={i} style={{ flex:1, height:3, borderRadius:99, background: i < score ? colors[score] : 'rgba(255,255,255,.1)', transition:'background .3s' }} />
        ))}
      </div>
      <div style={{ display:'flex', justifyContent:'space-between', alignItems:'center' }}>
        <div style={{ display:'flex', gap:8, flexWrap:'wrap' }}>
          {checks.map(c => (
            <span key={c.label} style={{ fontSize:10, color: c.ok ? '#34d399' : 'rgba(255,255,255,.25)', display:'flex', alignItems:'center', gap:3 }}>
      ...
      {[0,1,2,3,4,5].map(i => (
        <input
          ...
          style={{
            ...
            border: digits[i] ? '1.5px solid #818cf8' : '1.5px solid rgba(255,255,255,.12)',
            borderRadius:12, background: digits[i] ? 'rgba(129,140,248,.15)' : 'rgba(255,255,255,.06)',
          ...
          <React.Fragment key={s.id}>
            <div style={{ display:'flex', flexDirection:'column', alignItems:'center', gap:5 }}>
              <div style={{
                ...
                background: done ? '#4f46e5' : active ? 'rgba(129,140,248,.25)' : 'rgba(255,255,255,.06)',
                border: active ? '2px solid #818cf8' : done ? '2px solid #4f46e5' : '1.5px solid rgba(255,255,255,.12)',
                transition:'all .3s',
              }}>
                <span className="material-symbols-outlined" style={{ fontSize:15, color: done ? 'white' : active ? '#818cf8' : 'rgba(255,255,255,.3)' }}>
                  {done ? 'check' : s.icon}
                </span>
              </div>
              <span style={{ fontSize:10, fontWeight:600, color: active ? '#a5b4fc' : done ? 'rgba(255,255,255,.5)' : 'rgba(255,255,255,.2)', letterSpacing:'.03em' }}>{s.label}</span>
            </div>
            {i < steps.length - 1 && (
              <div style={{ width:48, height:1.5, background: done ? '#4f46e5' : 'rgba(255,255,255,.1)', marginBottom:20, transition:'background .3s' }} />
            )}
          </React.Fragment>
        );
      })}
    ...
    <>
      <style>{`
        *{box-sizing:border-box;margin:0;padding:0}
        ...
        .inp::placeholder{color:rgba(255,255,255,.25);font-size:13px}
        .inp:hover{border-color:rgba(255,255,255,.18);background:rgba(255,255,255,.09)}
        .inp:focus{border-color:#818cf8;background:rgba(255,255,255,.1);box-shadow:0 0 0 3px rgba(129,140,248,.18)}
        .inp-pw{padding-right:46px}
        .lbl{display:block;font-size:12px;font-weight:600;color:rgba(255,255,255,.5);margin-bottom:7px;letter-spacing:.02em}
        .btn{
        ...
        .btn:hover:not(:disabled){transform:translateY(-2px);box-shadow:0 10px 32px rgba(79,70,229,.5);animation:gradMove 2s infinite}
      ...
      <div style={{
        ...
        background:'linear-gradient(145deg,#0b1120 0%,#0f1c35 50%,#0d1629 100%)',
        position:'relative', overflow:'hidden', padding:'24px 16px',
      }}>
        {/* background glows */}
        <div style={{ position:'absolute', inset:0, pointerEvents:'none' }}>
          <div style={{ position:'absolute', top:'-15%', left:'-5%', width:'50%', height:'50%', borderRadius:'50%', background:'radial-gradient(circle,rgba(79,70,229,.18),transparent 70%)' }} />
          <div style={{ position:'absolute', bottom:'-10%', right:'-5%', width:'45%', height:'45%', borderRadius:'50%', background:'radial-gradient(circle,rgba(59,130,246,.13),transparent 70%)' }} />
          {/* dot grid */}
          <div style={{ position:'absolute', inset:0, opacity:.25, backgroundImage:'radial-gradient(circle,rgba(99,102,241,.5) 1px,transparent 1px)', backgroundSize:'28px 28px' }} />
        </div>
        ...
        <div style={{ width:'100%', maxWidth:440, position:'relative', zIndex:1 }}>
          ...
          <div style={{ display:'flex', alignItems:'center', justifyContent:'center', gap:10, marginBottom:32 }}>
            ...
            <span style={{ color:'rgba(255,255,255,.8)', fontSize:16, fontWeight:800 }}>WMS Portal</span>
          </div>
          ...
          <div style={{
            ...
            {step === 'SUCCESS' && (
              <div style={{ textAlign:'center', padding:'12px 0 8px', animation:'fadeUp .4s ease' }}>
                <div style={{ position:'relative', width:80, height:80, margin:'0 auto 20px' }}>
                  ...
                  <div style={{ width:80, height:80, borderRadius:'50%', background:'rgba(16,185,129,.18)', border:'2px solid rgba(16,185,129,.4)', display:'flex', alignItems:'center', justifyContent:'center', animation:'checkPop .5s cubic-bezier(.34,1.56,.64,1) both' }}>
                    <span className="material-symbols-outlined" style={{ color:'#34d399', fontSize:36 }}>check_circle</span>
                  </div>
                </div>
                <h2 style={{ color:'white', fontSize:22, fontWeight:800, marginBottom:8 }}>Đặt lại thành công!</h2>
                ...
                <div style={{ display:'flex', justifyContent:'center' }}>
                  <div style={{ width:160, height:3, borderRadius:99, background:'rgba(255,255,255,.1)', overflow:'hidden' }}>
                    <div style={{ height:'100%', background:'linear-gradient(90deg,#4f46e5,#34d399)', borderRadius:99, animation:'progress 2.8s linear forwards' }} />
                  </div>
                </div>
            ...
            {step === 'REQUEST_EMAIL' && (
              <div style={{ animation:'fadeUp .3s ease' }}>
                <div style={{ marginBottom:24 }}>
                  <h2 style={{ color:'white', fontSize:22, fontWeight:800, letterSpacing:'-.4px', marginBottom:6 }}>Quên mật khẩu?</h2>
                  <p style={{ color:'rgba(255,255,255,.35)', fontSize:13, lineHeight:1.6 }}>
                    Nhập email đã đăng ký — chúng tôi sẽ gửi mã xác minh 6 số.
                  </p>
                </div>

                {error && <div style={{ background:'rgba(239,68,68,.12)', border:'1px solid rgba(239,68,68,.25)', borderRadius:10, padding:'10px 13px', marginBottom:18, color:'#fca5a5', fontSize:13, display:'flex', gap:8, alignItems:'center' }}>
                ...
                <form onSubmit={handleRequestOTP} style={{ display:'flex', flexDirection:'column', gap:16 }}>
                  <div>
                    ...
                    <div style={{ position:'relative' }}>
                      <span className="material-symbols-outlined" style={{ position:'absolute', left:15, top:'50%', transform:'translateY(-50%)', fontSize:17, color:'rgba(255,255,255,.3)', pointerEvents:'none' }}>mail</span>
            ...
            {step === 'VERIFY_OTP' && (
              <div style={{ animation:'fadeUp .3s ease' }}>
                <div style={{ marginBottom:24 }}>
                  <h2 style={{ color:'white', fontSize:22, fontWeight:800, letterSpacing:'-.4px', marginBottom:6 }}>Kiểm tra email</h2>
                  <p style={{ color:'rgba(255,255,255,.35)', fontSize:13, lineHeight:1.6 }}>
                    Đã gửi mã 6 số tới <span style={{ color:'#a5b4fc', fontWeight:600 }}>{email}</span>
                  </p>
                </div>
                ...
                <form onSubmit={handleVerifyOTP} style={{ display:'flex', flexDirection:'column', gap:20 }}>
                  ...
                  <div style={{ textAlign:'center' }}>
                    <button type="button" onClick={handleResend} disabled={resendCountdown > 0 || loading}
                      style={{ background:'none', border:'none', cursor: resendCountdown > 0 ? 'default' : 'pointer', fontFamily:'inherit', fontSize:12, color: resendCountdown > 0 ? 'rgba(255,255,255,.25)' : '#818cf8', fontWeight:600, padding:0 }}>
            ...
            {step === 'RESET_PASSWORD' && (
              <div style={{ animation:'fadeUp .3s ease' }}>
                <div style={{ marginBottom:24 }}>
                  <h2 style={{ color:'white', fontSize:22, fontWeight:800, letterSpacing:'-.4px', marginBottom:6 }}>Đặt mật khẩu mới</h2>
                  <p style={{ color:'rgba(255,255,255,.35)', fontSize:13, lineHeight:1.6 }}>Tạo mật khẩu mạnh và không dùng lại mật khẩu cũ.</p>
                </div>
                ...
                <form onSubmit={handleResetPassword} style={{ display:'flex', flexDirection:'column', gap:16 }}>
                  <div>
                    ...
                    <div style={{ position:'relative' }}>
                      <span className="material-symbols-outlined" style={{ position:'absolute', left:15, top:'50%', transform:'translateY(-50%)', fontSize:17, color:'rgba(255,255,255,.3)', pointerEvents:'none' }}>lock</span>
                      ...
                      <button type="button" tabIndex={-1} onClick={() => setShowPw(v=>!v)}
                        style={{ position:'absolute', right:14, top:'50%', transform:'translateY(-50%)', background:'none', border:'none', cursor:'pointer', color:'rgba(255,255,255,.3)', display:'flex', padding:2 }}>
                  ...
                  <div>
                    ...
                    <div style={{ position:'relative' }}>
                      <span className="material-symbols-outlined" style={{ position:'absolute', left:15, top:'50%', transform:'translateY(-50%)', fontSize:17, color: confPw && confPw !== newPw ? 'rgba(239,68,68,.6)' : confPw && confPw === newPw ? 'rgba(52,211,153,.7)' : 'rgba(255,255,255,.3)', pointerEvents:'none' }}>lock</span>
                      <input className="inp inp-pw" type={showCf ? 'text' : 'password'} required
                        placeholder="Nhập lại mật khẩu"
                        style={{ borderColor: confPw && confPw !== newPw ? 'rgba(239,68,68,.4)' : confPw && confPw === newPw ? 'rgba(52,211,153,.4)' : undefined }}
                    ...
                    {confPw && confPw === newPw && (
                      <div style={{ marginTop:6, display:'flex', alignItems:'center', gap:5, color:'#34d399', fontSize:12 }}>
            ...
            {/* back link */}
            {step !== 'SUCCESS' && (
              <div style={{ marginTop:24, paddingTop:20, borderTop:'1px solid rgba(255,255,255,.07)', display:'flex', justifyContent:'center' }}>
                <Link
                  ...
                  style={{ display:'flex', alignItems:'center', gap:6, fontSize:12, color:'rgba(255,255,255,.3)', textDecoration:'none', fontWeight:500, transition:'color .15s' }}
